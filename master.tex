...
\begin{workSection}{Experience}
	...
	% note: chatgpt convo about accomplishments as of 2026-05-13
	\begin{itemize}
		\vspace{-0.5em}
		\itemsep -6pt {}
		% TODO: resolved the write applicant v2 endpoint that caused the risk event.
		% note: plan b of TIN certification date stuff
		\item Built \textbf{AWS Lambda Golang microservice}, monitored during 2-week oncall shifts, for internal clients to update customer tax withholding data at up to \textbf{200TPS} as it's migrated in phases to in-house DB.
		% \item Contributed to \& monitored during oncall an \textbf{AWS Lambda Golang microservice} for internal clients to update customer tax withholding data at up to \textbf{200TPS} as it's migrated in phases to in-house DB.
		\item Created \textbf{Claude Code} skill to automate onboarding tickets into code changes and GitHub PRs.
		% note: plan b, allowing it to monitor ^, though originally for discover to c1 data migration by conduit
		\item Wrote \textbf{Grafana K6} load test for QA-deployed Lambda simulating data migration load onto infrastructure.
		% TODO: find a better metric for the first bullet. my source wasn't correct
		% note: plan b until confirmation
		\item Added \textbf{OpenTelemetry} logging support across team microservices using Otelslog, cutting logging costs by \textbf{68\%} by migrating from Splunk to Observe and improving incident response through log correlation.
		% \item Created \textbf{Claude Code} skill to rapid
		% \item Participated in team on-call rotation, shadowing senior engineers to learn production incident escalation and operational workflows.
		% TODO: talk about the infrastructure behind the "internal db" 
		% TODO:
		% \item Implemented API contract changes across multiple \textbf{Go} microservices \& \textbf{OpenAPI} schemas supporting a phased migration from legacy to \textbf{AWS}-based customer data systems handling \textbf{6+M requests/week}.
	\end{itemize}
	...
	\begin{itemize}
		...
		\item Deployed, tested, and monitored production \textbf{Docker} infrastructure for semiconducctor wafer transporter robots, reducing projected fab operating costs by \textbf{$\sim$34\%}.
...
\begin{workSection}{Projects}
	...
	\begin{itemize}
		...
		\item Implemented production-grade two-stage (candidate-retrieval + ranking) \textbf{collaborative-filtering}-based movie recommender, deployed to \textbf{GPU-enabled AWS EC2}.
		\item Separated retrieval \& ranking services for independent scaling, connected by \textbf{gRPC} for minimum overhead.
		\item Used \textbf{FAISS} for GPU-accelerated candidate retrieval \& \textbf{pre-warmed, Redis + Postgres write-through cache} to minimize latency when fetching repeated embeddings.
	...
	\begin{itemize}
		\vspace{-0.5em}
		\itemsep -6pt {}
		% \item Built \textbf{multi-threaded} real-time collaborative editor in \textbf{Go}, handling concurrent edits from many users.
		\item Built \textbf{multi-threaded} real-time collaborative editor in \textbf{Go}, capable of \textbf{930+ edits/s} on Apple M4.
		\item Integrated \textbf{Automerge CRDTs} for conflict-free merging of simultaneous edits with \textbf{eventual consistency}.
		\item Designed event pipeline using \textbf{goroutines} and \textbf{WebSockets} to broadcast per-document updates.
		% \item Implemented \textbf{Pebble}-backed operation log with \textbf{crash-safe recovery} \& periodic full snapshots to bound replay time.
		% \item Created \textbf{Pebble}-backed operation log w/ \textbf{crash-safe recovery} \& periodic snapshots to cap replay time.
		% note: I forgot why I wrote "crash safe recovery"
		\item Created \textbf{Pebble}-backed operation log w/ \textbf{periodic full snapshots} to cap operation replay time.
	...
	\begin{itemize}
		...
		\item Allowed instant playback using \textbf{async} programming to \textbf{concurrently generate} summary \& play audio.
...
% 		skills=
% 		AWS Certified Solutions Architect - Associate (April 2026){,}
% 		Coursera Deep Learning (Oct 2023){,} %(adv.){,} 
% 	]
% 	\\
% 	\skillItem[
